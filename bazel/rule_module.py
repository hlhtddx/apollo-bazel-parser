...
class RuleModule(Module):
    ...
    def output_items(bp_file, items: Set[str], label: str, filter_func=None):
        ...
        if filter_func:
            names = set(filter_func(item) for item in items if filter_func(item))
        else:
            names = set(item for item in items)

        bp_file.write(f'    {label}: [\n')
        for name in sorted(names):
            bp_file.write(f'        "{name}",\n')
        bp_file.write(f'    ],\n')
    ...
    def output_modules(bp_file, items: Set[Module], label: str, filter_func=None):
        ...
        if filter_func:
            names = set(item.normal_name for item in items if filter_func(item))
        else:
            names = set(item.normal_name for item in items)
...
class CcModule(RuleModule):
    ...
    def post_load(self, modules):
        ...
        # Form the source file list
        self.sources = set(str(Path(self.path).joinpath(self._normalize_src_path(src))) for src in self.srcs)
    ...
    def parse(self, modules):
        for dep in self.deps:
            ...
            modules['libcyber'].merge(self)

    def merge(self, sub_module):
        self.sources.update(sub_module.sources)
        self.shared_libs.update(sub_module.shared_libs)
        logger.warning('shared_libs: %s', self.shared_libs)

    def output(self, bp_file):
        # Skip BUILD outside
        if not self.for_output:
            return
        ...
        bp_file.write(f'    compile_multilib: "64",\n')
        ...
        def filter_source_file(item):
            if item.endswith('.c') or item.endswith('.cc') or item.endswith('.cxx') \
                    or item.endswith('.S') or item.endswith('.s'):
                return item
            return None

        self.output_items(bp_file=bp_file, items=self.sources, label='srcs', filter_func=filter_source_file)
