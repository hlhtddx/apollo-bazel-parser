...
class CcModule(object):
    ...
    def output_to_android_bp(self, bp_file):
        ...
        if self.srcs:
            ...
            for i in self.srcs:
                bp_file.write(f'        "{self._normalize_src_path(i)}",\n')
            bp_file.write(f'    ],\n')
        #
        # if self.hdrs:
        #     bp_file.write(f'    export_header_lib_headers: [\n')
        #     for i in self.hdrs:
        #         bp_file.write(f'        "{self._normalize_src_path(i)}",\n')
        #     bp_file.write(f'    ],\n')
        ...
        #
        # if self.header_libs:
        #     bp_file.write(f'    header_libs: [\n')
        #     for i in self.header_libs:
        #         bp_file.write(f'        "{self._populate_lib_path(i)}",\n')
        #     bp_file.write(f'    ],\n')
...
def gen_android_bp_files(base_dir: str):
    for filepath, modules in cc_rules_by_path.items():
        if not filepath:
            continue
        bp_file_path = Path.joinpath(Path(base_dir), filepath, 'Android.bp')
